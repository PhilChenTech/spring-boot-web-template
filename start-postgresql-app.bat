...
if not "%PROFILE%"=="test" (
    ...
    powershell -Command "try { $connection = New-Object System.Data.SqlClient.SqlConnection; $connection.ConnectionString = 'Server=localhost;Database=postgres;Integrated Security=true;'; $connection.Open(); $connection.Close(); Write-Host 'PostgreSQL 連接正常' -ForegroundColor Green } catch { Write-Host '警告：無法連接到 PostgreSQL，請確認服務已啟動' -ForegroundColor Yellow }"
    echo.
)
